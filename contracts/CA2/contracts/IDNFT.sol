...
contract IDNFT6551 is ERC721, ERC721URIStorage, Ownable {
    ...
    event InjectiveModeSet(bool indexed injectiveMode);
    ...
    /**
     * @dev 通过Registry创建账户（外部调用，用于错误处理）
     * @param tokenId NFT ID
     * @return accountAddress 创建的账户地址
     */
    function _createAccountViaRegistry(uint256 tokenId) external returns (address accountAddress) {
        require(msg.sender == address(this), "IDNFT6551: Internal call only");
    ...
    /**
     * @dev 生成确定性地址（用于Injective模式）
     * @param tokenId NFT ID
     * @return accountAddress 生成的账户地址
     */
