...
async function main() {
  console.log("🧪 开始测试 Injective 测试网上的 CA4 合约（修复版）...");
  ...
  // 测试4: 检查Recipe NFT状态（使用已知token ID）
  console.log("\n4️⃣ 检查Recipe NFT状态");
  try {
    // 使用已知的token ID进行测试
    const tokenId = 1; // 部署时创建的token
  ...
  try {
    ...
    // 测试市场查询功能
    const forSaleTokens = await marketplaceContract.getForSaleTokens();
    console.log("✅ 市场上可授权的Recipe NFT数量:", forSaleTokens.length);
    
    if (forSaleTokens.length > 0) {
      console.log("✅ 可授权的Token IDs:", forSaleTokens.map(id => id.toString()));
    }
