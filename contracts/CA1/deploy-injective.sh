...
echo "🚀 Bars Help Bars - Injective 测试网部署脚本"
echo "================================================"
...
if [ ! -f .env ]; then
    ...
    echo "   特别是 ADMIN_PRIVATE_KEY 字段"
    exit 1
fi

# 检查私钥配置
if ! grep -q "ADMIN_PRIVATE_KEY" .env || grep -q "ADMIN_PRIVATE_KEY=" .env && ! grep -q "ADMIN_PRIVATE_KEY=0x" .env; then
    echo "❌ 错误: 请在 .env 文件中配置 ADMIN_PRIVATE_KEY"
    echo "   格式: ADMIN_PRIVATE_KEY=0x你的私钥"
    exit 1
fi
